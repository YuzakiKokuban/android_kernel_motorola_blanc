...
<p align="center">
<a href="https://github.com/YuzakiKokuban/android_kernel_motorola_blanc/releases"><img src="https://img.shields.io/github/v/release/YuzakiKokuban/android_kernel_motorola_blanc?style=for-the-badge&logo=github&color=blue" alt="GitHub release"></a>
...
这是一个面向 **摩托罗拉 Razr Fold** 的自定义内核项目，重点围绕稳定性、性能调优与日常可用性进行构建。项目当前维护 `LKM` 与 `ReSukiSU` 两种发行模式，以满足不同用户对纯净环境和高级功能的需求。
...
3. **刷入内核**：从本项目 [Releases 页面](https://github.com/YuzakiKokuban/android_kernel_motorola_blanc/releases) 下载对应版本，并在 Recovery 中刷入内核 `zip` 包。
...
所有最新构建版本均发布于 [**Releases 页面**](https://github.com/YuzakiKokuban/android_kernel_motorola_blanc/releases)。
...
# Kokuban Kernel for Motorola Razr Fold
...
This is a custom kernel project for the **Motorola Razr Fold**, built with a focus on stability, performance tuning, and day-to-day usability. The project currently maintains two release tracks, `LKM` and `ReSukiSU`, to serve both clean setups and advanced power-user workflows.
...
3. **Flash the Kernel**: Download the appropriate package from the [Releases page](https://github.com/YuzakiKokuban/android_kernel_motorola_blanc/releases) and flash the kernel `zip` through Recovery.
...
All current builds are published on the [**Releases Page**](https://github.com/YuzakiKokuban/android_kernel_motorola_blanc/releases).
